...
const upload = multer({
  ...
  fileFilter: (req: any, file: any, cb: any) => {
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      cb(new Error('Invalid file type. Only JPG, PNG, GIF, BMP, WEBP, and PDF files are allowed.'));
    }
  }
});
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/contacts/search", requireAuth, asyncHandler(async (req: Request, res: Response) => {
    ...
    const filteredContacts = await storage.searchContacts(criteria);
  ...
  // Delete contact
  app.delete("/api/contacts/:id", requireAuth, asyncHandler(async (req: Request, res: Response) => {
    const id = parseInt(req.params.id);
    if (isNaN(id)) {
      throw createError('Invalid contact ID', 400);
    }
  ...
  app.post("/api/business-cards/upload", requireAuth, upload.single('businessCard'), asyncHandler(async (req: Request, res: Response) => {
    if (!req.file) {
  ...
  app.get("/api/business-cards/:id/status", requireAuth, asyncHandler(async (req: Request, res: Response) => {
    const id = parseInt(req.params.id);
    if (isNaN(id)) {
  ...
  app.post("/api/business-cards/:id/verify", requireAuth, asyncHandler(async (req: Request, res: Response) => {
      ...
      res.status(200).json({ message: "Contact verified and created successfully", contact });
  }));

  // Get recent uploads with pagination
  app.get("/api/business-cards/recent", requireAuth, asyncHandler(async (req: Request, res: Response) => {
  ...
  app.get("/api/stats", requireAuth, asyncHandler(async (req: Request, res: Response) => {
  ...
  async function processBusinessCard(businessCardId: number, imagePath: string) {
    try {
      ...
      if (ocrConfidence < 0.5) {
        console.log(`OCR confidence is low (${ocrConfidence}), queuing for verification for business card ${businessCardId}`);
        await storage.updateBusinessCard(businessCardId, {
      ...
      console.log(`Confidence checks passed for business card ${businessCardId}`);
